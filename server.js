...
/**
 * --- B. Redis / Upstash Setup ---
 * Priority:
 *   1. Upstash REST (UPSTASH_REDIS_REST_URL + UPSTASH_REDIS_REST_TOKEN)
 *   2. Normal Redis instance (REDIS_URL, e.g. redis://localhost:6379)
 *   3. In-memory mock
 */
...
if (upstashUrl && upstashToken) {
  ...
  try {
    ...
    console.warn('[CACHE] Upstash Redis configuration failed! Falling back to other Redis options.', err.message);
  }
}

if (!isRedisConnected && redisTcpUrl) {
  // Fallback to normal Redis (container / local)
  try {
    redis = new IoRedis(redisTcpUrl); // e.g. redis://localhost:6379
    isRedisConnected = true;
    console.log('[CACHE] Successfully connected to Redis instance at', redisTcpUrl);
  } catch (err) {
    console.warn('[CACHE] Redis configuration failed! Falling back to IN-MEMORY REDIS MOCK.', err.message);
  }
}

if (!isRedisConnected) {
  console.warn('[SYSTEM] No Redis provider configured. Falling back to IN-MEMORY REDIS MOCK.');
}
...
app.post('/api/auth/signup', async (req, res) => {
  try {
    ...
    // Broadcast new user registration to all active WebSocket clients in real-time
    try {
      ...
      console.warn('[WS BROADCAST ERROR] Failed to broadcast new user signup:', wsErr.message);
    }
...
wss.on('connection', async (ws, req) => {
  ...
  ws.on('message', async (rawData) => {
    try {
      ...
      if (parsedData.type === 'get_history') {
        ...
        if (!(await checkGroupMembership(targetRoomId, ws.username))) {
      ...
      if (isMongoConnected) {
        const newDbMsg = new Message({ sender: ws.username, content: content.trim(), roomId: targetRoomId });
        await newDbMsg.save();
      } else {
        await localDbMock.saveMessage(ws.username, content.trim(), 'chat', targetRoomId);
      }
      ...
      if (targetRoomId === 'lounge') {
        ...
        if (otherUser.startsWith(`${ws.username}_`)) {
          otherUser = otherUser.substring(ws.username.length + 1);
        } else if (otherUser.endsWith(`_${ws.username}`)) {
          otherUser = otherUser.substring(0, otherUser.length - ws.username.length - 1);
        }

        [ws.username, otherUser].forEach(u => {
          const socketSet = userSockets.get(u);
          if (socketSet) {
            socketSet.forEach(s => {
  ...
  ws.on('close', async () => {
    const socketSet = userSockets.get(ws.username);
    if (socketSet) {
      socketSet.delete(ws);
      if (socketSet.size === 0) {
        userSockets.delete(ws.username);
        await client.srem('online_users', ws.username);
        console.log(`[WS] User ${ws.username} fully logged off.`);
        await broadcastOnlineUsers();
      } else {
        console.log(`[WS] Closed a tab for ${ws.username}. Remaining tabs: ${socketSet.size}`);
      }
    }
  });
});
